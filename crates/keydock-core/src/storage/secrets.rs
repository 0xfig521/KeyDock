...
impl AppStore {
    pub fn create_secret(&self, input: SecretInput) -> Result<Secret> {
        ...
        with_tx(&self.conn, |conn| {
            conn.execute(
                "INSERT INTO secrets (id, name, category, tags_json, notes, created_at, updated_at)
    ...
    pub fn update_secret(&self, id: &str, input: SecretInput) -> Result<Secret> {
        ...
        with_tx(&self.conn, |conn| {
            conn.execute(
                "UPDATE secrets SET name = ?2, category = ?3, tags_json = ?4,
                 notes = ?5, updated_at = ?6 WHERE id = ?1",
                params![
                    id,
                    &input.name,
                    input.category.as_str(),
    ...
    pub fn list_secrets(&self, limit: Option<usize>, offset: Option<usize>) -> Result<Vec<Secret>> {
        ...
        let mut stmt = self.conn.prepare(
            "SELECT id, name, category, tags_json, notes, created_at, updated_at
    ...
    pub fn get_secret_by_id(&self, id: &str) -> Result<Option<Secret>> {
        self.conn
            .query_row(
                "SELECT id, name, category, tags_json, notes, created_at, updated_at FROM secrets WHERE id = ?1",
                [id],
                row_to_secret,
            )
            .optional()
    ...
    pub fn get_secret_by_name(&self, name: &str) -> Result<Option<Secret>> {
        self.conn
            .query_row(
                "SELECT id, name, category, tags_json, notes, created_at, updated_at FROM secrets WHERE name = ?1",
                [name],
                row_to_secret,
            )
            .optional()
